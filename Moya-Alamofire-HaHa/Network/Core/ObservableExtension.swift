...
public extension Single where Element : Any {
    ...
    func mapHandyModel <T : HandyJSON> (type : T.Type) -> Single<T?> {
        return self.asObservable().mapHandyModel(type: type).asSingle()
    }
    
    // 将 Json 转成 模型数组
    func mapHandyModelArray<T: HandyJSON>(type: T.Type) -> Single<[T?]?> {
        return self.asObservable().mapHandyModelArray(type: type).asSingle()
    }
}
